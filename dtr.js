      ...
      setTimeout(() => {
        if (!cell.contains(document.activeElement)) {
          let timeVal = input.value
            ? formatTo12HourWithAMPM(input.value, select.value)
            : rawTime;
...
function formatTo12HourWithAMPM(time24, ampm) {
  if (!time24) return "";
